...
let iconArray = [<AuditIcon />,<WorkIcon />,<ApiIcon />,<NotificationIcon />,<ToolIcon />,<HandShake />, <Flag />, <People />, <Sdk/>, <Extension />, <Flag /> , <Sdk />, <Sdk />, <Blockchain/>, <Nodes />, <Verify />, <Extension/>, <Flag/>,<WorkIcon />,<WorkIcon />]
            ...
            <VerticalTimeline>
                {
                    timeLineElements.map(element => { 
                        if (element.id ===21){
                            return(
                                <VerticalTimelineElement key={element.key}iconStyle={{ background: 'rgb(16, 204, 82)', color: '#fff' }} icon={<Finish />} className="lastElement"/>
                            )
                        }
                        else
                        {
